      ...
      <div className="flex justify-between items-end">
        <div>
          <h2 className="text-4xl font-serif font-black text-white">
            Zarządzanie <span className="text-primary italic">Salami</span>
          </h2>
                   ...
                   <tr key={room.id} className="hover:bg-white/[0.02] transition-colors group">
                     <td className="p-6">
                       <span className="block font-bold text-white group-hover:text-primary transition-colors">
                         {room.room_number} <span className="text-slate-500 font-normal">({room.building_code})</span>
                       </span>
                       <span className="block text-xs text-slate-500 mt-1">{room.building_name}</span>
                     </td>
                     <td className="p-6">
                        <span className="block text-slate-300 font-medium">{room.room_type}</span>
                        <span className="block text-[10px] text-slate-500 mt-1 italic max-w-[200px] truncate" title={room.equipment_list}>
                          {room.equipment_list || 'Brak wyposażenia'}
                        </span>
                      </td>
                     <td className="p-6 text-center text-slate-300 font-bold">{room.capacity}</td>
                     ...
                     <td className="p-6 text-right">
                       <span className={`text-[9px] uppercase tracking-widest font-black px-3 py-1.5 rounded-full border ${
                         ...
                         room.status === 'under_maintenance' ? 'bg-amber-500/10 text-amber-400 border-amber-500/20' :
