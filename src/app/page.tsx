...
export default async function Home() {
  ...
  return (
    <div className="space-y-12 animate-in fade-in slide-in-from-bottom-4 duration-1000 ease-out">
      ...
      {/* Hero Section */}
      <div className="relative overflow-hidden rounded-3xl premium-gradient border border-border p-12">
        <div className="relative z-10 max-w-2xl">
          <h2 className="text-5xl font-serif font-black text-foreground leading-tight uppercase tracking-tight">
            Infrastruktura <span className="text-primary italic">Dydaktyczna AGH</span>
          </h2>
          <p className="text-muted-foreground mt-6 text-xl leading-relaxed font-medium">
            Centralny system zarządzania zasobami Akademii Górniczo-Hutniczej im. Stanisława Staszica w Krakowie.
          </p>
          <div className="flex gap-4 mt-8">
             <Link href="/schedules" className="px-8 py-3.5 bg-primary hover:bg-primary/90 text-primary-foreground font-bold rounded-xl transition-all active:scale-95 shadow-lg shadow-primary/20">
               Przeglądaj Harmonogramy
             </Link>
              <Link href="/equipment-analytics" className="px-8 py-3.5 bg-secondary hover:bg-secondary/80 text-foreground font-bold rounded-xl border border-border transition-all active:scale-95">
                Analiza Sprzętu
              </Link>
          </div>
        </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div className="glass-card p-8 rounded-2xl group hover:border-primary/50 transition-all duration-500">
          ...
          <h3 className="text-muted-foreground text-[10px] font-black uppercase tracking-[0.2em] mb-3">Ewidencja Sal</h3>
          <p className="text-5xl font-serif font-black text-foreground">{roomsCountResult.count}</p>
          <div className="h-1 w-12 bg-blue-500 mt-6 rounded-full group-hover:w-20 transition-all duration-500"></div>
          <p className="text-xs text-muted-foreground mt-4 leading-relaxed font-medium">Zarejestrowane obiekty dydaktyczne w {buildingsCount.count} budynkach</p>
        </div>
        ...
        <div className="glass-card p-8 rounded-2xl group hover:border-indigo-500/50 transition-all duration-500">
          ...
          <h3 className="text-muted-foreground text-[10px] font-black uppercase tracking-[0.2em] mb-3">Kadra Akademicka</h3>
          <p className="text-5xl font-serif font-black text-foreground">{profsCountResult.count}</p>
          <div className="h-1 w-12 bg-indigo-500 mt-6 rounded-full group-hover:w-20 transition-all duration-500"></div>
          <p className="text-xs text-muted-foreground mt-4 leading-relaxed font-medium">Kwalifikowani pracownicy naukowo-dydaktyczni</p>
        </div>
        ...
        <div className="glass-card p-8 rounded-2xl group hover:border-emerald-500/50 transition-all duration-500">
          ...
          <h3 className="text-muted-foreground text-[10px] font-black uppercase tracking-[0.2em] mb-3">Aktywne Rezerwacje</h3>
          <p className="text-5xl font-serif font-black text-foreground">{schedulesCountResult.count}</p>
          <div className="h-1 w-12 bg-emerald-500 mt-6 rounded-full group-hover:w-20 transition-all duration-500"></div>
          <p className="text-xs text-muted-foreground mt-4 leading-relaxed font-medium">Bieżące moduły lekcyjne w realizacji</p>
        </div>
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 items-start">
        {/* Kolumna boczna - Ostatnie obiekty */}
        <div className="lg:col-span-1 glass-card border-border shadow-2xl rounded-3xl overflow-hidden">
          <div className="px-8 py-6 border-b border-border bg-secondary/20">
            <h3 className="font-black text-foreground text-[10px] uppercase tracking-[0.3em]">Ostatnio dodane sale</h3>
          </div>
          <ul className="divide-y divide-border">
            {recentRooms.map((room, idx) => (
              <li key={idx} className="p-8 hover:bg-secondary/20 transition-all group">
                ...
                <div className="text-sm">
                   <span className="block text-foreground font-bold mb-1">{room.building_name}</span>
                   <span className="block text-muted-foreground text-xs italic">{room.campus_name}</span>
                </div>
              </li>
            ))}
          </ul>
          <div className="p-6 border-t border-border bg-secondary/20">
            <Link href="/schedules" className="flex items-center justify-center gap-2 py-3 w-full bg-secondary hover:bg-secondary/80 text-foreground text-xs font-black uppercase tracking-widest rounded-xl transition-all border border-border">
              Pełne zestawienie
        ...
        <div className="lg:col-span-2 glass-card border-border rounded-3xl p-10 md:p-14 relative overflow-hidden">
          <div className="absolute top-0 left-0 w-1.5 h-full bg-primary/50 pointer-events-none"></div>
          <h3 className="font-serif text-3xl font-black mb-10 text-foreground flex items-center gap-4">
            <span className="w-10 h-[1px] bg-primary/30"></span>
            Dokumentacja Systemowa
          </h3>
          <div className="space-y-8">
            <p className="text-foreground/80 text-xl leading-relaxed font-medium">
              Zintegrowany ekosystem informatyczny zaprojektowany do precyzyjnego zarządzania mieniem akademickim oraz optymalizacji procesów planowania zasobów uczelni.
            </p>
              ...
              <div className="space-y-4">
                 <div className="flex items-center gap-4 text-primary">
                    <div className="w-8 h-8 rounded-lg bg-primary/10 flex items-center justify-center border border-primary/20">
                     ...
                     <span className="text-xs font-black uppercase tracking-widest text-primary">Infrastruktura Cyfrowa</span>
                  </div>
                  <p className="text-muted-foreground text-sm leading-relaxed">Wydajne przetwarzanie danych z gwarancją integralności ACID.</p>
              </div>
              <div className="space-y-4">
                 <div className="flex items-center gap-4 text-indigo-400">
                  ...
                  <p className="text-muted-foreground text-sm leading-relaxed">Szyfrowane połączenia kaskadowe z pełnym audytem dostępu.</p>
              </div>
            </div>
          </div>
        </div>
