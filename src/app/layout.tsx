      ...
      <body className={`${inter.variable} ${serif.variable} bg-background text-foreground min-h-screen flex font-sans selection:bg-primary/30 overflow-hidden`}>
        ...
        <aside className="w-72 bg-sidebar border-r border-border flex flex-col z-40 h-screen sticky top-0 shadow-xl shadow-black/5">
          ...
          <div className="p-4 flex-1 overflow-y-auto custom-scrollbar space-y-2">
            ...
            {session?.role === 'admin' && (
              <>
                <p className="px-4 text-[10px] uppercase tracking-widest font-black text-sidebar-muted mb-2 mt-8">Administracja</p>
                <Link href="/admin" className="flex items-center gap-3 px-4 py-3 rounded-xl hover:bg-secondary/80 text-sidebar-foreground transition-all group active:scale-95">
                  <div className="w-8 h-8 rounded-lg bg-secondary group-hover:bg-indigo-500/20 group-hover:text-indigo-400 flex items-center justify-center transition-colors">
                  ...
                  <span className="text-sm font-semibold">Panel Administratora</span>
                </Link>
              </>
            )}
